#ifndef UMP_SOCK_PRIVATE_DEF
#define UMP_SOCK_PRIVATE_DEF
...
static void ump_handle_ctrl(UMPSocket* u_sock,glong* sleep_ms);//由sock_thread_func调用，再去调用状态机处理函数
static void ump_end_ctrl(UMPSocket* u_sock,UMPCallMethod call_method, gboolean ctrl_result);
static void ump_handle_ctrl_packet(UMPSocket* u_sock,glong* sleep_ms);//由sock_thread_func调用，再去调用状态机处理函数
static void ump_handle_ctrl_timeout(UMPSocket* u_sock,glong* sleep_ms);
static void ump_handle_send_ctrl_packet(UMPSocket* u_sock,glong* sleep_ms);//由sock_thread_func调用，发出准备好的ctrl包并捎带ctrl_ack，或单独发送ctrl_ack
...
static void ump_handle_send(UMPSocket* u_sock,glong* sleep_ms);//由sock_thread_func调用，处理发送调用
...
typedef struct _ump_socket
{
	...
	GMutex* public_state_lock;//保护public_state、connect_time、close_time
